...
try:
    ...
    # create a camera dispatcher thread
    camera = CameraController(tello=tello)
    # camera.run_front_cam()
    camera.run_bottom_cam()

    # Start the periodic drone state logger thread
    state_logger = threading.Thread(target=log_state, args=(state_logging_interval, tello), daemon=True,
                                    name='state-logger')
    state_logger.start()
